 ...
 * @author Marek Fukala
 */
public class AdaEmbeddingProvider extends EmbeddingProvider {

    public static final String GENERATED_CODE = "@@@"; //NOI18N

    @Override
    public List<Embedding> getEmbeddings(Snapshot snapshot) {
        TokenHierarchy<CharSequence> th = TokenHierarchy.create(snapshot.getText(), AdaTokenId.language());
        TokenSequence<AdaTokenId> sequence = th.tokenSequence(AdaTokenId.language());

        //issue #159775 logging >>>
        if(sequence == null) {
            Logger.getLogger("AdaEmbeddingProvider").warning(
                    "TokenHierarchy.tokenSequence(AdaTokenId.language()) == null " +
                    "for static immutable Ada TokenHierarchy!\nFile = '"+
        ...
        while (sequence.moveNext()) {
            ...
            if (t.id() == AdaTokenId.PACKAGE || t.id() == AdaTokenId.PROCEDURE || t.id() == AdaTokenId.FUNCTION) {
                if(from < 0) {
                ...
                if(from >= 0) {
                    ...
                    embeddings.add(snapshot.create(from, len, "text/x-ada")); //NOI18N
                    //add only one virtual generated token for a sequence of PHP tokens
                    embeddings.add(snapshot.create(GENERATED_CODE, "text/x-ada"));
                }
    ...
    public static final class Factory extends TaskFactory {
        ...
        public Collection<SchedulerTask> create(final Snapshot snapshot) {
            return Collections.<SchedulerTask>singletonList(new AdaEmbeddingProvider());
        }
    }
}
