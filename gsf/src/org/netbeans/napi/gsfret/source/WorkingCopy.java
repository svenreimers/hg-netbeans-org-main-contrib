...
public class WorkingCopy extends CompilationController {
    ...
    public List<Difference> getChanges() throws IOException {
        return diffs;
    }
    ...
    public void addDiff(Difference diff) {
        ...
        if (diffs == null) {
            ...
            diffStarts = new HashSet<Integer>();
        }
        // Make sure we don't enter overlapping sections.
        // For example, in Ruby, "foo + 42" will create TWO
        // nodes referencing the "foo" symbol: a LocalVarNode
        // to first read "foo", and then a LocalAsgnNode to write
        // the new value. These are overlapping and I can only
        // have a single reference here.
        if (!diffStarts.contains(start)) {
            diffs.add(diff);
            diffStarts.add(start);
        }
    }
}
